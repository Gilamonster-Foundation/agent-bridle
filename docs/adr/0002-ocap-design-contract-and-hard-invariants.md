...
| **INV-BOUND (I15)** | A spawned child's **actual applied authority is bounded by the effective caveats** of the authorizing `ToolContext`: on every axis, the authority the enforcement mechanism actually permits is `⊑ (delegated ∪ explicitly-declared runtime closure)`, and the fence the admission decision analyzes **is the exact fence applied** — `admitted == applied`, no post-admission re-derivation. Any widening the mechanism introduces beyond that bound — profile *expansion* (launcher variants, `base_read_paths`/loader paths, rootfs closures) or authority *substitution* (egress-proxy) — MUST be an explicit, minimal, **harness-disjoint declared closure** element, or be **refused** (`ScopeRelation ∈ {Superset, Incomparable, Unknown} ⇒ refuse`). This is the **enforcement-grain corollary of I2/I3**: the meet/no-amplify guarantee that holds in the `Caveats` algebra MUST survive to the applied mechanism. ADR 0015's *deny-more-only* is its sandbox-side statement. | The scope operand ships — `agent-bridle-core::provenance` re-exports `agent_mesh_protocol::{ResolvedScope, ScopeRelation, admit}` (#329) — and the per-axis strength floor ships (`EnforcementFloor`, #317). The live admission→application wiring — one `AdmittedFence` that is *both* analyzed and applied, `admit(resolved, delegated, declared_closure)` refusing `Superset`/`Unknown`, and a `CID(applied) == admitted` pre-spawn guard — is #136. | **Partial** — scope operand + strength floor are on `main`, but admission still analyzes a *narrower* object than the applied kernel ruleset (`base_read_paths`/loader/launcher/rootfs closures are added **after** admission), so `admitted == applied` does not yet hold. #136 closes it. |
