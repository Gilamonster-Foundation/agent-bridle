...
use crate::{
    best_available_sandbox, Caveats, SandboxKind, Scope, ToolContext, ToolError, ToolResult,
};
...
impl ConfinedCommand {
    ...
    pub fn spawn(self, cx: &ToolContext) -> ToolResult<ConfinedChild> {
        ...
        //     child inherits the (per-thread, fork/exec-inherited) Landlock domain.
        let Self {
            program,
            args,
            envs,
...
mod tests {
    ...
    /// Off-Linux / no-landlock builds: a restrictive `fs_write` must be refused
    /// rather than spawned unconfined. (On a landlock build this path doesn't
    /// fire — the kernel enforces — so the test is gated off there.)
