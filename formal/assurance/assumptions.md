...
| **ASM-PROBE-LIVENESS** | A green native probe means the mechanism was OBSERVED, not merely that nothing crashed. The register can prove a `native_test_fn` **resolves** (validate.sh 1-7) and that the gate validating it can **trigger** (#356/#362); it cannot prove the probe **ran** on the certifying run — a skip, a child that never started, or an assertion satisfied by empty output all report the same green and exit 0. Worked example: newt-agent#1687, a kernel-enforcement conclusion drawn from a PowerShell child that never executed. **Enforcement today is declarative, not executable:** every claim citing native evidence MUST carry a `liveness` field naming its control (a capability gate, a `REQUIRE_*` hard-fail, a positive assertion, an unguarded control spawn) or stating WEAK and why — validate.sh 8 fails the gate on silence. That forces the question at authoring time; it does not verify the control works. | `formal/assurance/validate.sh` section 8 (declaration required) + each claim's `liveness` field | **held / registered residual** — the executable half (a probe emitting a liveness artifact carrying the SHA it observed, so "evidence for claim X on SHA Y" is checkable rather than inferred) is agent-bridle#361 and undone. The one WEAK declaration today is MACOS-E4's mach-floor differential (~6 silent early returns). |
...
genuinely run on the Linux test host. Even a green E4 characterization discharges only the
