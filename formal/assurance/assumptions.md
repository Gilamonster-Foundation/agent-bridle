...
| **ASM-POSIX-DESCRIPTOR** | A confined child obtains a resource only by inheriting a handle (fd / preopened dir), never by pathname resolution against an ambient root — i.e. `fds ⊑ effective` holds on the REAL path. **Now PARTIAL (#319 slice 1):** every confined-spawn site (core `ConfinedCommand`, the `OsSpawner` shell stages, carried-coreutils) closes ambient descriptors `>= 3` before `exec` via `agent-bridle-fdguard::deny_inherited_fds` (async-signal-safe `close_range`, encapsulated so core stays `forbid(unsafe_code)`), preserving stdio and failing closed on error. **Enforcement is Linux-only** today; macOS/Windows still rely on the CLOEXEC convention (tracked follow-up). The `/proc/self/fd` reopen and `O_PATH`-upgrade re-mediation (§4 of the model) is separate and still design. | `agent-bridle-tool-shell/tests/real_spawn.rs::real_ambient_fd_is_not_inherited` (ambient CLOEXEC-cleared file absent from the confined child's `/proc/self/fd`) + `agent-bridle-fdguard` unit test | **partial** — Linux real-path closure landed + native probe on main; macOS/Windows residual + RC-cert pending |
