...
| **ASM-POSIX-DESCRIPTOR** | A confined child obtains a resource only by inheriting a handle (fd / preopened dir), never by pathname resolution against an ambient root — i.e. `fds ⊑ effective` holds on the REAL path. **Now PARTIAL (#319 slice 1):** every confined-spawn site (core `ConfinedCommand`, the `OsSpawner` shell stages, carried-coreutils) closes ambient descriptors `>= 3` before `exec` via `agent-bridle-fdguard::deny_inherited_fds` (async-signal-safe: `close_range` on Linux, a bounded `fcntl(F_SETFD, FD_CLOEXEC)` sweep on macOS — encapsulated so core stays `forbid(unsafe_code)`), preserving stdio and failing closed on error. **Enforcement covers Linux and macOS**; on Windows the confined path spawns via aclaunch's explicit `PROC_THREAD_ATTRIBUTE_HANDLE_LIST` (only delegated handles inheritable), while a `std::process` Windows spawn still relies on the handle-inheritance convention (residual). The `/proc/self/fd` reopen and `O_PATH`-upgrade re-mediation (§4 of the model) is separate and still design. | `agent-bridle-tool-shell/tests/real_spawn.rs::{real_ambient_fd_is_not_inherited, real_ambient_fd_is_not_inherited_macos}` (ambient CLOEXEC-cleared descriptor absent/unusable in the confined child, with an unguarded positive control) + `agent-bridle-fdguard` unit tests | **partial** — Linux + macOS real-path closure; std-path Windows residual + `/proc`-reopen re-mediation + RC-cert pending |
